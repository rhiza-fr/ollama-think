...
class MyResponseObject(BaseModel):
    ...
    how_hot_is_the_world: str = Field(..., description="your reasoning for the response")
    average_temperature: float = Field(..., description="average temperature")
    confidence: int = Field(..., description="how confident you are about your respponse. 0-10")
...
def main():    
    with Client(host="http://localhost:11434") as client:
        ...
        text_json = client.call(model="qwen3", prompt="Design a json representation of a spiral galaxy", format="json").content
        print(text_json)
        ...
        text_obj = client.call(model="qwen3", prompt="How hot is the world?", format=MyResponseObject.model_json_schema()).content
