...
def main():    
    ...
    print("Calling model...")
    response = client.call(model="qwen3", prompt="Is concentration of capital a good thing?", think=True)
    print(response)
    ...
    thinking, content = client.call(model="qwen3", prompt="The future is here it is just not evenly distributed: Discuss...", think=True)
    print(f"Thinking: {thinking}")
    print(f"\nContent: {content}")

    # using a traditional message
    messages = [{"role": "user", "content": "Make a system prompt for feng shui analysis"}]
